...
exports.init = function init(startup_time, config, events, logger) {
  ...
  commandsPerBatch = config.atsd.commandsPerBatch;
  ...
  commandsPerBatch = commandsPerBatch !== undefined ? commandsPerBatch : 100;
...
function parse(key) { // match metric name against pattern and parse according to atsd-pattern if matches
  ...
  for (var k = 0; k < patterns.length; k++) {
    ...
    var regExp = pattern.pattern instanceof RegExp ? pattern.pattern : new RegExp('' + pattern.pattern);
  ...
  if (atsdHost) {
    try {
      ...
      for (var c = 0; c < commands.length; c++) {
        ...
        if ((c + 1) % commandsPerBatch === 0 || c === commands.length - 1) {
          if (protocol.toLowerCase() === 'udp') {
