...
exports.init = function init(startup_time, config, events, logger) {
    ...
    if (protocol.toLowerCase() === "udp") {
        ...
        netConnection = net.createConnection(atsdPort, atsdHost);

        netConnection.addListener(
            "error", function (connectionException) {
                if (debug) {
                    log.log(connectionException);
                }
            }
        );
...
function postStats(command) { // create a connection and flush data into ATSD
...
*/
            ...
            stats.flush_length = command.length;
